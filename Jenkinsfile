  ...
  stages {
    ...
    stage('Deploy sphinx plugin') {
      ...
      steps {
        ...
        sh 'export ANSIBLE_CONFIG="/tmp/sphinx-plugin-deployment/ansible.cfg" ansible-playbook -i /tmp/sphinx-plugin-deployment/hosts /tmp/sphinx-plugin-deployment/$COIN_TYPE-config.yml'
      }
    }
  }
